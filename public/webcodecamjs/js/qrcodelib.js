...
GridSampler.checkAndNudgePoints = function(image, points) {
    ...
    for (var offset = 0; offset < points.length && nudged; offset += 2) {
        ...
        nudged = false;
        if (x == -1) { points[offset] = 0;
            nudged = true } else if (x == width) { points[offset] = width - 1;
            nudged = true }
        if (y == -1) { points[offset + 1] = 0;
            nudged = true } else if (y == height) {
            points[offset + 1] = height - 1;
            nudged = true
        }
    }
    nudged = true;
    for (var offset = points.length - 2; offset >= 0 && nudged; offset -= 2) { var x = Math.floor(points[offset]); var y = Math.floor(points[offset + 1]); if (x < -1 || x > width || y < -1 || y > height) throw "Error.checkAndNudgePoints ";
        nudged = false; if (x == -1) { points[offset] = 0;
            nudged = true } else if (x == width) { points[offset] = width - 1;
            nudged = true } if (y == -1) { points[offset + 1] = 0;
            nudged = true } else if (y == height) { points[offset + 1] = height - 1;
            nudged = true } }
};
GridSampler.sampleGrid3 = function(image, dimension, transform) {
    ...
    for (var y = 0; y < dimension; y++) {
        ...
        var iValue = y + .5;
        for (var x = 0; x < max; x += 2) { points[x] = (x >> 1) + .5;
            points[x + 1] = iValue }
...
function ECB(count, dataCodewords) { this.count = count;
...
function Version(versionNumber, alignmentPatternCenters, ecBlocks1, ecBlocks2, ecBlocks3, ecBlocks4) {
    ...
    for (var i = 0; i < ecbArray.length; i++) { var ecBlock = ecbArray[i];
        total += ecBlock.Count * (ecBlock.DataCodewords + ecCodewords) }
    this.totalCodewords = total;
    this.__defineGetter__("VersionNumber",
        function() { return this.versionNumber });
    ...
    this.buildFunctionPattern = function() {
        ...
        for (var x = 0; x < max; x++) { var i = this.alignmentPatternCenters[x] - 2; for (var y = 0; y < max; y++) { if (x == 0 && (y == 0 || y == max - 1) || x == max - 1 && y == 0) continue;
                bitMatrix.setRegion(this.alignmentPatternCenters[y] - 2, i, 5, 5) } }
        bitMatrix.setRegion(6, 9, 1, dimension - 17);
        bitMatrix.setRegion(9, 6, dimension - 17, 1);
        if (this.versionNumber > 6) { bitMatrix.setRegion(dimension - 11, 0, 3, 6);
...
Version.getVersionForNumber = function(versionNumber) { if (versionNumber < 1 || versionNumber > 40) throw "ArgumentException"; return Version.VERSIONS[versionNumber - 1] };
...
Version.decodeVersionInformation = function(versionBits) {
    ...
    var bestVersion = 0;
    for (var i = 0; i < Version.VERSION_DECODE_INFO.length; i++) { var targetVersion = Version.VERSION_DECODE_INFO[i]; if (targetVersion == versionBits) return this.getVersionForNumber(i + 7); var bitsDifference = FormatInformation.numBitsDiffering(versionBits, targetVersion); if (bitsDifference < bestDifference) { bestVersion = i + 7;
...
function PerspectiveTransform(a11, a21, a31, a12, a22, a32, a13, a23, a33) {
    ...
    this.transformPoints2 = function(xValues, yValues) { var n = xValues.length; for (var i = 0; i < n; i++) { var x = xValues[i]; var y = yValues[i]; var denominator = this.a13 * x + this.a23 * y + this.a33;
...
PerspectiveTransform.squareToQuadrilateral = function(x0, y0, x1, y1, x2, y2, x3, y3) { dy2 = y3 - y2;
    dy3 = y0 - y1 + y2 - y3; if (dy2 == 0 && dy3 == 0) return new PerspectiveTransform(x1 - x0, x2 - x1, x0, y1 - y0, y2 - y1, y0, 0, 0, 1);
    else { dx1 = x1 - x2;
        dx2 = x3 - x2;
        dx3 = x0 - x1 + x2 - x3;
        dy1 = y1 - y2;
        ...
        a23 = (dx1 * dy3 - dx3 * dy1) / denominator; return new PerspectiveTransform(x1 - x0 + a13 * x1, x3 - x0 + a23 * x3, x0, y1 - y0 + a13 * y1, y3 - y0 + a23 * y3, y0, a13, a23, 1) } };
...
function DetectorResult(bits, points) { this.bits = bits;
    this.points = points }
...
function Detector(image) {
    ...
    this.sizeOfBlackWhiteBlackRunBothWays = function(fromX, fromY, toX, toY) {
        ...
        if (otherToX <
            0) { scale = fromX / (fromX - otherToX);
            otherToX = 0 } else if (otherToX >= qrcode.width) { scale = (qrcode.width - 1 - fromX) / (otherToX - fromX);
            otherToX = qrcode.width - 1 }
        var otherToY = Math.floor(fromY - (toY - fromY) * scale);
        scale = 1;
        if (otherToY < 0) { scale = fromY / (fromY - otherToY);
            otherToY = 0 } else if (otherToY >= qrcode.height) { scale = (qrcode.height - 1 - fromY) / (otherToY - fromY);
    ...
    this.calculateModuleSizeOneWay =
        function(pattern, otherPattern) { var moduleSizeEst1 = this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(pattern.X), Math.floor(pattern.Y), Math.floor(otherPattern.X), Math.floor(otherPattern.Y)); var moduleSizeEst2 = this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(otherPattern.X), Math.floor(otherPattern.Y), Math.floor(pattern.X), Math.floor(pattern.Y)); if (isNaN(moduleSizeEst1)) return moduleSizeEst2 / 7; if (isNaN(moduleSizeEst2)) return moduleSizeEst1 / 7; return (moduleSizeEst1 + moduleSizeEst2) / 14 };
    this.calculateModuleSize =
        function(topLeft, topRight, bottomLeft) { return (this.calculateModuleSizeOneWay(topLeft, topRight) + this.calculateModuleSizeOneWay(topLeft, bottomLeft)) / 2 };
    this.distance = function(pattern1, pattern2) { xDiff = pattern1.X - pattern2.X;
        yDiff = pattern1.Y - pattern2.Y; return Math.sqrt(xDiff * xDiff + yDiff * yDiff) };
    ...
    this.createTransform = function(topLeft, topRight, bottomLeft, alignmentPattern,
        ...
        if (alignmentPattern != null) { bottomRightX = alignmentPattern.X;
            bottomRightY = alignmentPattern.Y;
            sourceBottomRightX = sourceBottomRightY = dimMinusThree - 3 } else { bottomRightX = topRight.X - topLeft.X + bottomLeft.X;
    ...
    this.processFinderPatternInfo = function(info) {
        ...
        if (provisionalVersion.AlignmentPatternCenters.length > 0) {
            ...
            for (var i = 4; i <= 16; i <<= 1) { alignmentPattern = this.findAlignmentInRegion(moduleSize, estAlignmentX, estAlignmentY, i); break }
        }
...
function FormatInformation(formatInfo) { this.errorCorrectionLevel = ErrorCorrectionLevel.forBits(formatInfo >> 3 & 3);
...
function ErrorCorrectionLevel(ordinal, bits, name) { this.ordinal_Renamed_Field = ordinal;
    this.bits = bits;
    this.name = name;
...
ErrorCorrectionLevel.forBits = function(bits) { if (bits < 0 || bits >= FOR_BITS.length) throw "ArgumentException"; return FOR_BITS[bits] };
...
function BitMatrix(width, height) {
    ...
    this.get_Renamed = function(x, y) { var offset = y * this.rowSize + (x >> 5); return (URShift(this.bits[offset], x & 31) & 1) != 0 };
    this.set_Renamed = function(x, y) { var offset = y * this.rowSize + (x >> 5);
        this.bits[offset] |= 1 << (x & 31) };
    this.flip = function(x, y) { var offset = y * this.rowSize + (x >> 5);
        this.bits[offset] ^= 1 << (x & 31) };
    ...
    this.setRegion = function(left, top, width, height) {
        ...
        for (var y = top; y < bottom; y++) { var offset = y * this.rowSize; for (var x = left; x < right; x++) this.bits[offset + (x >> 5)] |= 1 << (x & 31) }
    }
}

function DataBlock(numDataCodewords, codewords) { this.numDataCodewords = numDataCodewords;
...
DataBlock.getDataBlocks = function(rawCodewords, version, ecLevel) {
    ...
    while (longerBlocksStartAt >= 0) { var numCodewords = result[longerBlocksStartAt].codewords.length; if (numCodewords == shorterBlocksTotalCodewords) break;
        longerBlocksStartAt-- }
    longerBlocksStartAt++;
...
function BitMatrixParser(bitMatrix) {
    ...
    this.copyBit = function(i, j, versionBits) { return this.bitMatrix.get_Renamed(i, j) ? versionBits << 1 | 1 : versionBits << 1 };
    ...
    this.readCodewords = function() {
        ...
        var currentByte =
            0;
        var bitsRead = 0;
        for (var j = dimension - 1; j > 0; j -= 2) { if (j == 6) j--; for (var count = 0; count < dimension; count++) { var i = readingUp ? dimension - 1 - count : count; for (var col = 0; col < 2; col++)
                    if (!functionPattern.get_Renamed(j - col, i)) { bitsRead++;
                        currentByte <<= 1; if (this.bitMatrix.get_Renamed(j - col, i)) currentByte |= 1; if (bitsRead == 8) { result[resultOffset++] = currentByte;
                            bitsRead = 0;
                            currentByte = 0 } } }
            readingUp ^= true }
...
DataMask = {};
DataMask.forReference = function(reference) { if (reference < 0 || reference > 7) throw "System.ArgumentException"; return DataMask.DATA_MASKS[reference] };

function DataMask000() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { return (i + j & 1) == 0 } }

function DataMask001() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { return (i & 1) == 0 } }

function DataMask010() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { return j % 3 == 0 } }

function DataMask011() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { return (i + j) % 3 == 0 } }

function DataMask100() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { return (URShift(i, 1) + j / 3 & 1) == 0 } }

function DataMask101() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { var temp = i * j; return (temp & 1) + temp % 3 == 0 } }

function DataMask110() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
    this.isMasked = function(i, j) { var temp = i * j; return ((temp & 1) + temp % 3 & 1) == 0 } }

function DataMask111() { this.unmaskBitMatrix = function(bits, dimension) { for (var i = 0; i < dimension; i++)
            for (var j = 0; j < dimension; j++)
                if (this.isMasked(i, j)) bits.flip(j, i) };
...
function ReedSolomonDecoder(field) {
    ...
    this.decode = function(received, twoS) {
        ...
        var noError = true;
        for (var i = 0; i < twoS; i++) { var eval = poly.evaluateAt(this.field.exp(dataMatrix ? i + 1 : i));
            syndromeCoefficients[syndromeCoefficients.length - 1 - i] = eval; if (eval != 0) noError = false }
        if (noError) return;
    ...
    this.runEuclideanAlgorithm = function(a, b, R) {
        if (a.Degree < b.Degree) { var temp = a;
            a = b;
            b = temp }
        var rLast = a;
        var r = b;
        ...
        while (r.Degree >= Math.floor(R / 2)) {
            ...
            while (r.Degree >= rLast.Degree && !r.Zero) { var degreeDiff = r.Degree - rLast.Degree; var scale = this.field.multiply(r.getCoefficient(r.Degree), dltInverse);
    ...
    this.findErrorLocations = function(errorLocator) { var numErrors = errorLocator.Degree; if (numErrors == 1) return new Array(errorLocator.getCoefficient(1)); var result = new Array(numErrors); var e = 0; for (var i = 1; i < 256 && e < numErrors; i++)
            if (errorLocator.evaluateAt(i) == 0) { result[e] = this.field.inverse(i);
                e++ }
        if (e != numErrors) throw "Error locator degree does not match number of roots"; return result };
    this.findErrorMagnitudes = function(errorEvaluator,
        errorLocations, dataMatrix) { var s = errorLocations.length; var result = new Array(s); for (var i = 0; i < s; i++) { var xiInverse = this.field.inverse(errorLocations[i]); var denominator = 1; for (var j = 0; j < s; j++)
                if (i != j) denominator = this.field.multiply(denominator, GF256.addOrSubtract(1, this.field.multiply(errorLocations[j], xiInverse)));
            result[i] = this.field.multiply(errorEvaluator.evaluateAt(xiInverse), this.field.inverse(denominator)); if (dataMatrix) result[i] = this.field.multiply(result[i], xiInverse) } return result }
}
...
function GF256Poly(field, coefficients) {
    ...
    this.evaluateAt = function(a) {
        ...
        if (a == 1) { var result = 0; for (var i = 0; i < size; i++) result = GF256.addOrSubtract(result, this.coefficients[i]); return result }
    ...
    this.addOrSubtract = function(other) {
        ...
        if (smallerCoefficients.length > largerCoefficients.length) { var temp = smallerCoefficients;
    ...
    this.multiply2 = function(scalar) { if (scalar == 0) return this.field.Zero; if (scalar == 1) return this; var size = this.coefficients.length; var product = new Array(size); for (var i = 0; i < size; i++) product[i] = this.field.multiply(this.coefficients[i], scalar); return new GF256Poly(this.field, product) };
...
function GF256(primitive) {
    ...
    var x = 1;
    for (var i = 0; i < 256; i++) { this.expTable[i] = x;
        x <<= 1; if (x >= 256) x ^= primitive }
    ...
    this.buildMonomial =
        function(degree, coefficient) { if (degree < 0) throw "System.ArgumentException"; if (coefficient == 0) return zero; var coefficients = new Array(degree + 1); for (var i = 0; i < coefficients.length; i++) coefficients[i] = 0;
            coefficients[0] = coefficient; return new GF256Poly(this, coefficients) };
    this.exp = function(a) { return this.expTable[a] };
    this.log = function(a) { if (a == 0) throw "System.ArgumentException"; return this.logTable[a] };
    this.inverse = function(a) { if (a == 0) throw "System.ArithmeticException"; return this.expTable[255 - this.logTable[a]] };
    this.multiply = function(a, b) { if (a == 0 || b == 0) return 0; if (a == 1) return b; if (b == 1) return a; return this.expTable[(this.logTable[a] + this.logTable[b]) % 255] }
}
...
qrcode.decode = function(src) {
    if (arguments.length == 0) { var canvas_qr = qrcode.sourceCanvas ? qrcode.sourceCanvas : document.getElementById("qr-canvas"); var context = canvas_qr.getContext("2d");
        qrcode.width = canvas_qr.width;
        qrcode.height = canvas_qr.height;
        qrcode.imagedata = context.getImageData(0, 0, qrcode.width, qrcode.height);
        qrcode.result = qrcode.process(context); if (qrcode.callback != null) qrcode.callback(qrcode.result); return qrcode.result } else {
        var image = new Image;
        image.onload = function() {
            ...
            if (image.width * image.height > qrcode.maxImgSize) { var ir = image.width / image.height;
            ...
            qrcode.width = canvas_qr.width;
            qrcode.height = canvas_qr.height;
            try { qrcode.imagedata = context.getImageData(0, 0, canvas_qr.width, canvas_qr.height) } catch (e) {
                qrcode.result = "Cross domain image reading not supported in your browser! Save it to your computer then drag and drop the file!";
                if (qrcode.callback != null) qrcode.callback(qrcode.result);
                return
            }
            try { qrcode.result = qrcode.process(context) } catch (e) { qrcode.result = "error decoding QR Code" }
            if (qrcode.callback != null) qrcode.callback(qrcode.result)
        };
        image.src = src
    }
...
qrcode.decode_url = function(s) { var escaped = ""; try { escaped = escape(s) } catch (e) { escaped = s } var ret = ""; try { ret = decodeURIComponent(escaped) } catch (e) { ret = escaped } return ret };
qrcode.decode_utf8 = function(s) { if (qrcode.isUrl(s)) return qrcode.decode_url(s);
    else return s };
qrcode.process = function(ctx) {
    var start = (new Date).getTime();
    var image = qrcode.grayScaleToBitmap(qrcode.grayscale());
    if (qrcode.debug) { for (var y = 0; y < qrcode.height; y++)
            for (var x = 0; x < qrcode.width; x++) { var point = x * 4 + y * qrcode.width * 4;
...
qrcode.getPixel = function(x, y) { if (qrcode.width < x) throw "point error"; if (qrcode.height < y) throw "point error";
    point = x * 4 + y * qrcode.width * 4;
    p = (qrcode.imagedata.data[point] * 33 + qrcode.imagedata.data[point + 1] * 34 + qrcode.imagedata.data[point + 2] * 33) / 100; return p };
qrcode.binarize = function(th) { var ret = new Array(qrcode.width * qrcode.height); for (var y = 0; y < qrcode.height; y++)
        for (var x = 0; x < qrcode.width; x++) { var gray = qrcode.getPixel(x, y);
...
qrcode.getMiddleBrightnessPerArea = function(image) {
    ...
    for (var i = 0; i < numSqrtArea; i++) { minmax[i] = new Array(numSqrtArea); for (var i2 = 0; i2 < numSqrtArea; i2++) minmax[i][i2] = new Array(0, 0) }
...
function URShift(number, bits) { if (number >= 0) return number >> bits;
...
qrcode.orderBestPatterns = function(patterns) {
    function distance(pattern1, pattern2) { xDiff = pattern1.X - pattern2.X;
        yDiff = pattern1.Y - pattern2.Y; return Math.sqrt(xDiff * xDiff + yDiff * yDiff) }
    ...
    if (oneTwoDistance >= zeroOneDistance && oneTwoDistance >= zeroTwoDistance) { pointB = patterns[0];
        pointA = patterns[1];
        pointC = patterns[2] } else if (zeroTwoDistance >= oneTwoDistance && zeroTwoDistance >= zeroOneDistance) { pointB = patterns[1];
        pointA = patterns[0];
        pointC = patterns[2] } else { pointB = patterns[2];
        pointA = patterns[0];
        pointC = patterns[1] }
    if (crossProductZ(pointA, pointB, pointC) < 0) { var temp = pointA;
        pointA = pointC;
        pointC = temp }
    patterns[0] = pointA;
    patterns[1] = pointB;
...
function FinderPattern(posX, posY, estimatedModuleSize) {
    ...
    this.aboutEquals = function(moduleSize, i, j) {
        if (Math.abs(i - this.y) <=
            moduleSize && Math.abs(j - this.x) <= moduleSize) { var moduleSizeDiff = Math.abs(moduleSize - this.estimatedModuleSize); return moduleSizeDiff <= 1 || moduleSizeDiff / this.estimatedModuleSize <= 1 }
        return false
    }
}

function FinderPatternInfo(patternCenters) { this.bottomLeft = patternCenters[0];
...
function FinderPatternFinder() {
    ...
    this.centerFromEnd = function(stateCount, end) { return end - stateCount[4] - stateCount[3] - stateCount[2] / 2 };
    this.crossCheckVertical = function(startI, centerJ, maxCount, originalStateCountTotal) {
        var image = this.image;
        var maxI = qrcode.height;
        var stateCount = this.CrossCheckStateCount;
        var i = startI;
        while (i >= 0 && image[centerJ + i * qrcode.width]) { stateCount[2]++;
            i-- }
        if (i < 0) return NaN;
        while (i >= 0 && !image[centerJ + i * qrcode.width] && stateCount[1] <= maxCount) { stateCount[1]++;
            i-- }
        if (i <
            0 || stateCount[1] > maxCount) return NaN;
        while (i >= 0 && image[centerJ + i * qrcode.width] && stateCount[0] <= maxCount) { stateCount[0]++;
            i-- }
        if (stateCount[0] > maxCount) return NaN;
        i = startI + 1;
        while (i < maxI && image[centerJ + i * qrcode.width]) { stateCount[2]++;
            i++ }
        if (i == maxI) return NaN;
        while (i < maxI && !image[centerJ + i * qrcode.width] && stateCount[3] < maxCount) { stateCount[3]++;
            i++ }
        if (i == maxI || stateCount[3] >= maxCount) return NaN;
        while (i < maxI && image[centerJ + i * qrcode.width] && stateCount[4] < maxCount) { stateCount[4]++;
            i++ }
        if (stateCount[4] >=
            maxCount) return NaN;
    ...
    this.crossCheckHorizontal = function(startJ, centerI, maxCount, originalStateCountTotal) {
        ...
        if (j < 0) return NaN;
        while (j >= 0 && !image[j + centerI * qrcode.width] && stateCount[1] <= maxCount) { stateCount[1]++;
            j-- }
        if (j < 0 || stateCount[1] > maxCount) return NaN;
        while (j >= 0 && image[j + centerI * qrcode.width] && stateCount[0] <= maxCount) { stateCount[0]++;
            j-- }
        if (stateCount[0] > maxCount) return NaN;
        j = startJ + 1;
        while (j < maxJ && image[j + centerI * qrcode.width]) { stateCount[2]++;
            j++ }
        if (j == maxJ) return NaN;
        while (j < maxJ && !image[j + centerI * qrcode.width] && stateCount[3] < maxCount) { stateCount[3]++;
            j++ }
        if (j == maxJ || stateCount[3] >= maxCount) return NaN;
        while (j < maxJ && image[j + centerI * qrcode.width] && stateCount[4] < maxCount) { stateCount[4]++;
            j++ }
            ...
            if (!isNaN(centerJ)) {
                ...
                for (var index = 0; index < max; index++) {
                    ...
                    if (center.aboutEquals(estimatedModuleSize,
                            centerI, centerJ)) { center.incrementCount();
                        found = true; break }
                }
                if (!found) { var point = new FinderPattern(centerJ, centerI, estimatedModuleSize);
                    this.possibleCenters.push(point); if (this.resultPointCallback != null) this.resultPointCallback.foundPossibleResultPoint(point) }
                return true
            }
        }
        return false
    ...
    this.selectBestPatterns = function() {
        ...
        if (startSize > 3) {
            ...
            this.possibleCenters.sort(function(center1, center2) { var dA = Math.abs(center2.EstimatedModuleSize - average); var dB = Math.abs(center1.EstimatedModuleSize - average); if (dA < dB) return -1;
            ...
            for (var i = 0; i < this.possibleCenters.length && this.possibleCenters.length >
                3; i++) { var pattern = this.possibleCenters[i]; if (Math.abs(pattern.EstimatedModuleSize - average) > limit) { this.possibleCenters.splice(i, 1);
                    i--; } }
        }
        if (this.possibleCenters.length > 3) this.possibleCenters.sort(function(a, b) { if (a.count > b.count) return -1; if (a.count < b.count) return 1; return 0 });
    ...
    this.findRowSkip = function() {
        ...
        for (var i = 0; i < max; i++) {
            ...
            if (center.Count >= CENTER_QUORUM)
                ...
                else { this.hasSkipped = true; return Math.floor((Math.abs(firstConfirmedCenter.X - center.X) - Math.abs(firstConfirmedCenter.Y - center.Y)) / 2) }
        }
        return 0
    };
    this.haveMultiplyConfirmedCenters = function() {
        ...
        for (var i = 0; i < max; i++) { pattern = this.possibleCenters[i];
    ...
    this.findFinderPattern = function(image) {
        ...
        for (var i = iSkip - 1; i < maxI && !done; i += iSkip) {
            ...
            for (var j = 0; j < maxJ; j++)
                if (image[j + i * qrcode.width]) { if ((currentState & 1) == 1) currentState++;
                    stateCount[currentState]++ } else if ((currentState & 1) == 0)
                if (currentState == 4)
                    if (this.foundPatternCross(stateCount)) {
                        ...
                        if (confirmed) {
                            ...
                            else { var rowSkip = this.findRowSkip(); if (rowSkip > stateCount[2]) { i += rowSkip - stateCount[2] - iSkip;
                                    j = maxJ - 1 } }
                        } else { do j++; while (j < maxJ && !image[j + i * qrcode.width]);
                            j-- }
                        currentState = 0;
                        stateCount[0] = 0;
                        stateCount[1] = 0;
                        ...
                        stateCount[3] = 0;
                        stateCount[4] = 0
                    } else { stateCount[0] = stateCount[2];
            ...
            if (this.foundPatternCross(stateCount)) {
                ...
                if (confirmed) { iSkip = stateCount[0]; if (this.hasSkipped) done = haveMultiplyConfirmedCenters() }
            }
        }
...
function AlignmentPatternFinder(image, startX, startY, width, height, moduleSize, resultPointCallback) {
    ...
    this.centerFromEnd = function(stateCount, end) { return end - stateCount[2] - stateCount[1] / 2 };
    this.foundPatternCross = function(stateCount) {
    ...
    this.crossCheckVertical = function(startI, centerJ, maxCount, originalStateCountTotal) {
        ...
        while (i >= 0 && !image[centerJ + i * qrcode.width] &&
            stateCount[0] <= maxCount) { stateCount[0]++;
            i-- }
        if (stateCount[0] > maxCount) return NaN;
        i = startI + 1;
        while (i < maxI && image[centerJ + i * qrcode.width] && stateCount[1] <= maxCount) { stateCount[1]++;
            i++ }
        if (i == maxI || stateCount[1] > maxCount) return NaN;
        while (i < maxI && !image[centerJ + i * qrcode.width] && stateCount[2] <= maxCount) { stateCount[2]++;
            i++ }
        ...
        for (var iGen = 0; iGen < height; iGen++) {
            ...
            while (j < maxJ) {
                if (image[j + i * qrcode.width])
                    ...
                    if (this.foundPatternCross(stateCount)) { var confirmed = this.handlePossibleCenter(stateCount, i, j); if (confirmed != null) return confirmed }
...
function QRCodeDataBlockReader(blocks, version, numErrorCorrectionCode) {
    ...
    this.getNextBits = function(numBits) {
        ...
        if (numBits < this.bitPointer + 1) {
            ...
            return bits
        } else if (numBits < this.bitPointer + 1 + 8) { var mask1 = 0; for (var i = 0; i < this.bitPointer + 1; i++) mask1 += 1 << i;
            bits = (this.blocks[this.blockPointer] & mask1) << numBits - (this.bitPointer + 1);
            this.blockPointer++;
            bits += this.blocks[this.blockPointer] >> 8 - (numBits - (this.bitPointer + 1));
            this.bitPointer = this.bitPointer - numBits % 8; if (this.bitPointer < 0) this.bitPointer = 8 + this.bitPointer; return bits } else if (numBits < this.bitPointer +
            1 + 16) {
            var mask1 = 0;
            var mask3 = 0;
    ...
    this.getDataLength = function(modeIndicator) { var index = 0; while (true) { if (modeIndicator >> index == 1) break;
            index++ } return this.getNextBits(qrcode.sizeOfDataLengthInfo[this.dataLengthMode][index]) };
    this.getRomanAndFigureString =
        function(dataLength) {
            ...
            do
                if (length > 1) { intData = this.getNextBits(11); var firstLetter = Math.floor(intData / 45); var secondLetter = intData % 45;
                    strData += tableRomanAndFigure[firstLetter];
                    strData += tableRomanAndFigure[secondLetter];
                    length -= 2 } else if (length ==
                1) { intData = this.getNextBits(6);
                strData += tableRomanAndFigure[intData];
                length -= 1 } while (length > 0);
            return strData
        };
    this.getFigureString = function(dataLength) { var length = dataLength; var intData = 0; var strData = "";
        do { if (length >= 3) { intData = this.getNextBits(10); if (intData < 100) strData += "0"; if (intData < 10) strData += "0";
                length -= 3 } else if (length == 2) { intData = this.getNextBits(7); if (intData < 10) strData += "0";
                length -= 2 } else if (length == 1) { intData = this.getNextBits(4);
                length -= 1 }
            strData += intData } while (length > 0); return strData };
    this.get8bitByteArray = function(dataLength) { var length = dataLength; var intData = 0; var output = new Array;
        do { intData = this.getNextBits(8);
            output.push(intData);
            length-- } while (length > 0); return output };
